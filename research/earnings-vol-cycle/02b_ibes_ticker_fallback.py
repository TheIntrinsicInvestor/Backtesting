...
For the 14 permnos that 02_earnings_dates.py missed (CUSIP mismatch between CRSP and IBES),
...
# ── Permno → ticker lookup (for the 14 missing) ───────────────────────────────
perm_to_ticker = missing_df.set_index("permno")["ticker"].to_dict()
ticker_to_perm = {}
for permno, ticker in perm_to_ticker.items():
    ticker_to_perm.setdefault(ticker, permno)  # one permno per ticker (first wins)

# Disambiguate TFCF / TFCFA: both are Twenty-First Century Fox
# TFCFA is the voting Class A share (primary); assign TFCFA -> permno 90441
# TFCF  is Class B; map IBES ticker FOX (if present) -> 90442
# Use actual CRSP tickers as-is; IBES may carry them directly
...
# ── Map ticker -> permno ──────────────────────────────────────────────────────
# For TFCF / TFCFA both have their own permno; check which IBES tickers we got
df_new["permno"] = df_new["ticker"].map(ticker_to_perm)

# Attempt CUSIP-based mapping for any rows where ticker match is ambiguous
cusip_to_permno = (
